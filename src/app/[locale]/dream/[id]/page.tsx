...
import { findDreamWithDetails } from "@/lib/data-store";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";

export default async function DreamDetailPage({ params }: { params: { locale: string; id: string } }) {
  const result = findDreamWithDetails(params.id);
  if (!result) return notFound();
  const { dream, interpretation, videos } = result;
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          {interpretation && (
            <div className="space-y-3">
              <div className="flex gap-2">
                <Badge variant="success">Ton: {interpretation.tone}</Badge>
                <Badge variant="outline">Dil: {interpretation.lang}</Badge>
              </div>
          ...
          <form action={`/api/dreams/${dream.id}/video`} method="post" className="flex gap-3">
            ...
            {videos[0]?.outputUrl && (
              <Button variant="outline" href={videos[0].outputUrl} target="_blank" rel="noreferrer">
                mp4 indir
              </Button>
            )}
          </form>
